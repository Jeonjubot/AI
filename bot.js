...
async function handleSend() {
  ...
  if (next) {
    if (["END", "END1", "END2"].includes(next.다음ID)) {
      showRating();
    } else {
...
async function submitRating() {
  ...
  try {
   ...
   showMessage("감사합니다!");
    ...
    if (["예비1", "예비2", "예비3"].includes(lastID)) {
      let message = "TEST1 관련 문의입니다.";

      if (lastID === "예비1") {
        message = "TEST2 관련 문의입니다.";
      } else if (lastID === "예비2") {
        message = "TEST3 관련 문의입니다.";
      } else if (lastID === "예비3") {
        message = "TEST4 관련 문의입니다.";
      }
